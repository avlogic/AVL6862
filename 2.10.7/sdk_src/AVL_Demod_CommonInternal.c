...
AVL_ErrorCode InitSemaphore_Demod(AVL_uint32 uiChipNo)
{
    ...
    static AVL_uchar gReceiver_sem_inited = 0;          //receiver semaphore initialization indicator
    static AVL_uchar gDisqec_sem_inited = 0;            //Disqec operation semaphore indicator
    static AVL_uchar gI2C_sem_inited = 0;               //I2C operation semaphore indicator

    if( 0 == gReceiver_sem_inited )
    {
        gReceiver_sem_inited = 1;
        r |= AVL_IBSP_InitSemaphore(&(gstChipInternalArray[uiChipNo].semRx));
    }
    if( 0 == gDisqec_sem_inited )
    {
        gDisqec_sem_inited = 1;
...
AVL_ErrorCode AVL_ParseFwPatch_v0(AVL_uint32 uiChipNo, AVL_uchar download_only) {
    ...
    //#define PARSER_DEBUG
        ...
        //System::Console::WriteLine("num_cmd: {0} condition: {1} cmd: {2} r: {3}",cmd_count++ ,condition,cmd_str,r);
        //System::Console::WriteLine("num_cmd: {0} condition: {1} cmd: {2}",cmd_count++,condition,AVL_patch_read32(pPatchData, &patch_idx,0));
    #endif
        if(condition) 
        {
            ...
            switch(cmd) 
            {
                ...
                case PATCH_CMD_DECOMPRESS://4
                {
                    ...
                    AVL_uint32 num_rvs = AVL_patch_read32(pPatchData, &patch_idx,1);
                    patch_idx += 4*(num_rvs); //no RV's defined yet
