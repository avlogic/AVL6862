...
DMD_u32_t DMD_API DMD_open( DMD_PARAMETER* param)
{
	...
	param->freq		= 666000000; //hz

	//ts output method select 
	param->ts_out_mode = DMD_E_TSOUT_PARALLEL_BRTG_MODE; //20140512, troy.wangyx, added TS output mode and TS clk polarity selection.
...
DMD_u32_t DMD_API DMD_set_system( DMD_PARAMETER* param ){
	...
	if( DMD_set_ts_output(param, param->ts_out_mode, DMD_E_TSCLK_POLARITY_NORMAL) == DMD_E_ERROR ){
...
DMD_u32_t DMD_API DMD_scan( DMD_PARAMETER* param )
{
	...
	printk("pxydy0205, DMD_scan 1\n");
	ret = DMD_device_scan( param );
	printk("pxydy0205, DMD_scan 2\n");
	return ret;
}
