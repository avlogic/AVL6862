...
extern unsigned char ucPatchData [];
...
{
	{
			...
			AVL_DVBSx_SA_0,
			Ref_clock_27M,
#ifdef CONFIG_AVL6211_OUTPUT_SERIAL
...
AVL_DVBSx_ErrorCode AVL6211_Initialize(struct AVL_DVBSx_Chip * pAVLChip,struct AVL_Tuner * pTuner)
{
	...
	r = AVL_DVBSx_IBase_Initialize(&(pll_conf[pDemodTunerConfig->m_DemodReferenceClk]), (AVL_puchar)ucPatchData, pAVLChip); 
	if( AVL_DVBSx_EC_OK != r ) 
	{
