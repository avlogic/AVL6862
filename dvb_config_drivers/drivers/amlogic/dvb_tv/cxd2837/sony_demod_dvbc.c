/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2013-03-22 13:52:22 #$
  File Revision : $Revision:: 6791 $
------------------------------------------------------------------------------*/
...
sony_result_t sony_demod_dvbc_Tune (sony_demod_t * pDemod)
{
	...
	printk("[sony_demod_dvbc.c]: sony_demod_dvbc_Tune(): state(%d) system(%d)\n", pDemod->state, pDemod->system);
...
#ifdef DVBC_6M_BAND
        pDemod->bandwidth = SONY_DEMOD_BW_6_MHZ;
#else
	...
	printk("[sony_demod_dvbc.c]: sony_demod_dvbc_Tune(): system(%d)[C(%d)] bandwidth(%d)[%d/%d]\n", pDemod->system, SONY_DTV_SYSTEM_DVBC, pDemod->bandwidth, SONY_DEMOD_BW_6_MHZ, SONY_DEMOD_BW_8_MHZ);
    SONY_TRACE_RETURN (result);
}
...
{
    sony_result_t result = SONY_RESULT_OK;
	printk("[sony_demod_dvbc.c]: sony_demod_dvbc_Sleep()\n");
                                              ...
                                              sony_demod_lock_result_t * pLock)
{
	...
	printk("[sony] mode(%d) lockCheck (%d)(%d)(%d)\n", pDemod->scanMode, arLock, tsLock, unlockDetected);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
{
    sony_result_t result = SONY_RESULT_OK;
	printk("[sony_demod_dvbc.c]: SLTtoAC()\n");
...
{
    sony_result_t result = SONY_RESULT_OK;
	printk("[sony_demod_dvbc.c]: ACtoAC()\n");
...
{
    sony_result_t result = SONY_RESULT_OK;
	printk("[sony_demod_dvbc.c]: ACtoSLT()\n");
