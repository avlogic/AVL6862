...
------------------------------------------------------------------------------*/
/**
 @file    sony_example_dvbc_scan.c

         This file provides an example of DVB-C channel scanning
*/
/*------------------------------------------------------------------------------
 Includes
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
static void scanCallbackHandler (sony_integ_t * pInteg, sony_integ_dvbc_scan_result_t * pResult, sony_integ_dvbc_scan_param_t * pScanParam)
{
    ...
    if (pResult->tuneResult == SONY_RESULT_OK) {
        ...
        if (newResult != NULL) {
            newResult->next = NULL;
            newResult->prev = NULL;
            newResult->tuneParam = pResult->tuneParam;
...
int main (int argc, char *argv[])
{
    ...
    sony_i2c_t i2c;
    sony_integ_dvbc_scan_param_t scanParam;
    ...
    /* Configure the DVBC scan parameters based on a full spectrum.  Modify the following
       parameters specific to the scan requirements. */
    scanParam.startFrequencyKHz = 650000;   /* Start frequency of scan process */
    scanParam.endFrequencyKHz = 680000;     /* End frequency of scan process */
    ...
    tuneResult = sony_integ_dvbc_Scan (&integ, &scanParam, &scanCallbackHandler);
    ...
    if (getChannelCount() > 0) {
        ...
        * ------------------------------------------------------------------------------ */
        ...
        printf ("  - Center Frequency   : %uKHz\n", pScanResultsHead->tuneParam.centerFreqKHz);

        /* Perform DVBC Tune using the scan results tune param structure directly */
        tuneResult = sony_integ_dvbc_Tune (&integ, &pScanResultsHead->tuneParam);
