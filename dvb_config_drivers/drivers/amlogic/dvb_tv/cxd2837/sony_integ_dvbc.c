/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2013-03-26 11:11:48 #$
  File Revision : $Revision:: 6829 $
------------------------------------------------------------------------------*/
                                   ...
                                   sony_dvbc_tune_param_t * pTuneParam)
{
	...
	printk("[sony_integ_dvbc.c]: sony_integ_dvbc_Tune(): \n");
    ...
    #endif
   {
	...
	result = Mxl603SetFreqBw(8,pTuneParam->centerFreqKHz);
	#else
		#ifdef DVBC_6M_BAND /* (2015.07.23) [EOM] 6MHz Bandwidth */
        result = pInteg->pTunerTerrCable->Tune (pInteg->pTunerTerrCable, pTuneParam->centerFreqKHz, SONY_DTV_SYSTEM_DVBC, SONY_DEMOD_BW_6_MHZ);
		#else
...
static sony_result_t dvbc_WaitDemodLock (sony_integ_t * pInteg)
{
    ...
    uint32_t elapsed = 0;
#if 1 /* (2015.06.02)(EOM) Wait a short time for lock check */
    uint32_t notdetect = 0;
#endif
