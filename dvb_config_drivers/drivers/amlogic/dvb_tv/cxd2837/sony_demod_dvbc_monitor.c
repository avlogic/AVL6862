...
//	printk("[aaaa] result(%i) ar(%i) ts(%i) ea(%i)\n\n", result, ar, tslock, earlyUnlock);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
sony_result_t sony_demod_dvbc_monitor_Quality (sony_demod_t * pDemod, uint32_t * pQuality)
{
    uint32_t ber = 0;
    int32_t snr = 0;
    int32_t snRel = 0;
	...
	*pQuality = 0;
    ...
    if ((result != SONY_RESULT_OK) || (snr == 0)){
        SONY_TRACE_RETURN (result);
    }
    ...
    if (snRel < -7 * 1000) {
        ...
        *pQuality = (uint32_t) (((tmpSQI * berSQI) + (1000000/2)) / (1000000)) & 0xFF;
    }
    else {
        *pQuality = (uint32_t) ((berSQI + 500) / 1000);
    }
